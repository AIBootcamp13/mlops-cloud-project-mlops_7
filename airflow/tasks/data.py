...
def prepare_data(random_state: int) -> dict[str, str]:
    ...
    train_df, val_df, test_df = prepareddata.split_dataset(features)
    train_x, train_y = prepareddata.prepare_features_target(train_df)
    val_x, val_y = prepareddata.prepare_features_target(val_df)
    test_x, test_y = prepareddata.prepare_features_target(test_df)
